...
SYSTEM_PROMPT = """你是一个能执行本地 shell 命令的 Agent。
当用户提出任务时，你会自主决定下一步：
- 如果需要执行命令，请严格以 "命令:XXX" 的格式回复，其中 XXX 是你要执行的命令（每次只执行一条）。
- 如果任务已经完成，请严格以 "完成:XXX" 的格式回复，其中 XXX 是对结果的总结。
- 不要输出解释、分析或任何额外文字，只输出 "命令:..." 或 "完成:...”。

执行命令时请注意安全，不要执行可能破坏系统的命令。"""
...
def main():
    ...
    print("Agent 已启动，输入任务，输入 'exit'、'quit' 或 '退出' 可结束。\n")
