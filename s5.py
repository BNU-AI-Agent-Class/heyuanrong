...
def main():
    ...
    agent_path = os.path.join(base_dir, "agent.md")
    skill_path = os.path.join(base_dir, "skill.md")

    if not os.path.exists(agent_path):
        print(f"[错误] 找不到 {agent_path}，请先创建 agent.md。")
        sys.exit(1)
    if not os.path.exists(skill_path):
        print(f"[错误] 找不到 {skill_path}，请先创建 skill.md。")
        sys.exit(1)

    system_prompt = load_prompt(agent_path) + "\n\n" + load_prompt(skill_path)
    messages = [{"role": "system", "content": system_prompt}]

    print("领域 Agent 已启动（agent.md + skill.md），输入任务，输入 'exit'、'quit' 或 '退出' 可结束。\n")
